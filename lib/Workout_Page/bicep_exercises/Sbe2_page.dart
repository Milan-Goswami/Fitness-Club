...
class Sbe2_page extends StatefulWidget {
  final String passedValue;
  const Sbe2_page({super.key, required this.passedValue});

  @override
  State<Sbe2_page> createState() => _Sbe2_pageState();
}

class _Sbe2_pageState extends State<Sbe2_page> {
 ...
 final Map<String, double> exerciseMet = {
  ...
  void _onNextExercise({bool isLast = false}) {
    int secondsSpent = _stopwatch.elapsed.inSeconds;
    final String currentExerciseName = content4[CurrentIndex].title;
    double exerciseCals =
    ...
    if (isLast) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (context) => be2_page()),
      );
    } else {
      setState(() {
        CurrentIndex++;
