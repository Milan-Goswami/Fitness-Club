...
class Sin2_page extends StatefulWidget {
  final String passedValue;
  const Sin2_page({super.key, required this.passedValue});

  @override
  State<Sin2_page> createState() => _Sin2_pageState();
}

class _Sin2_pageState extends State<Sin2_page> {
  ...
  final Map<String, double> exerciseMet = {
    "BICEP CURL": 3.0,
    "BICEP HARD CURL": 3.2,
    "BICEP CURL DUMPBLE": 3.0,
    "90 DEGREE DUMPBLE": 3.1,
  };
  ...
  void _onNextExercise({bool isLast = false}) {
    int secondsSpent = _stopwatch.elapsed.inSeconds;
    final String currentExerciseName = content5[CurrentIndex].title;
    double exerciseCals =
    ...
    if (isLast) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (context) => in2_page()),
      );
    } else {
      setState(() {
        CurrentIndex++;
