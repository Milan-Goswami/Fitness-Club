...
class sad2_page extends StatefulWidget {
  final String passedValue;
  const sad2_page({super.key, required this.passedValue});

  @override
  State<sad2_page> createState() => _sad2_pageState();
}

class _sad2_pageState extends State<sad2_page> {
  ...
  void _onNextExercise({bool isLast = false}) {
    int secondsSpent = _stopwatch.elapsed.inSeconds;
    final String currentExerciseName = content6[CurrentIndex].title;
    double exerciseCals =
    ...
    if (isLast) {
      ...
      Navigator.pushReplacement(
        context,
        MaterialPageRoute(builder: (context) => Ad2_page()),
      );
    } else {
      setState(() {
        CurrentIndex++;
